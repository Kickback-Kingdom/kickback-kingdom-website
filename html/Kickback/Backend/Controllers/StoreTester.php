...
class StoreTester
{
    public static function testStoreController()
    {
        ...
        echo "Uncomment Tests To Preform Them";
    }
    ...
    private static function test_apply_override_coupon_checkout_cart() : void
    {
        ...
        try
        {
            ...
            $mockCouponPrice = static::createMockCouponPrice($mockPriceItem, 1);
            if(count($mockCouponPrice) !== 1) throw new Exception("COMPONENT TEST FAILED : expected exaclty one price object in mock coupon price list");

            $mockOverridePriceItem = static::createMockOverrideCouponPriceItem($mockMedia);
            $mockOverrideCouponPrice = static::createMockCouponPrice($mockOverridePriceItem, 1);
            if(count($mockOverrideCouponPrice) !== 1) throw new Exception("COMPONENT TEST FAILED : expected exaclty one price object in mock coupon price list");
            ...
            //CREATE COUPONS
                    ...
                    //OVERRIDE COUPON
            $overrideCoupon = static::createMockInfiniteOverrideCoupon($mockProduct, $mockOverrideCouponPrice);
            if(count($overrideCoupon->price) !== count($mockOverrideCouponPrice)) throw new Exception("COMPONENT TEST FAILED : override coupon does not have all of the mockOverrideCouponPrice. Expected : ".count($mockOverrideCouponPrice)." | Actual : ".count($overrideCoupon->price));
            if(is_null($overrideCoupon)) throw new Exception("COMPONENT TEST FAILED : failed to return mock coupon after creation");

            if(!static::doesCouponHaveLinksToPrice($overrideCoupon, $mockOverrideCouponPrice)) throw new Exception("COMPONENT TEST FAILED : override coupon does not have expected links");
            
                    //COUPON
            $coupon = static::createMockInfiniteCoupon($mockProduct, $mockCouponPrice);
            if(count($coupon->price) !== count($mockCouponPrice)) throw new Exception("COMPONENT TEST FAILED : coupon does not have all of the mockCouponPrice. Expected : ".count($mockCouponPrice)." | Actual : ".count($coupon->price));
            if(is_null($coupon)) throw new Exception("COMPONENT TEST FAILED : failed to return mock coupon after creation");
            
            if(!static::doesCouponHaveLinksToPrice($coupon, $mockCouponPrice)) throw new Exception("COMPONENT TEST FAILED : coupon does not have expected links");
            ...
            $doesCartHaveCouponPrice = static::doesCartItemHaveCouponPrice($cart->cartProducts[0], $coupon);
            if(!$doesCartHaveCouponPrice) throw new Exception("COPMONENT TEST FAILED : cart item price do not match coupon price");
            ...
            $doesCartHaveCouponPrice = static::doesCartItemHaveCouponPrice($cart->cartProducts[0], $overrideCoupon);
            if(!$doesCartHaveCouponPrice) throw new Exception("COPMONENT TEST FAILED : cart item price do not match override coupon price");
            ...
            $doesAccountHaveLootForPrice = static::doesAccountHavePriceOfProduct($mockProduct, $mockSellerAccount);
            if(!$doesAccountHaveLootForPrice) throw new Exception("COMPONENT TEST FAILED : seller did not receive loots for product price");
        ...
        {
            static::cleanupTestEnviroment($database);
            throw new Exception("Exception while testing appying, overriding, and checking out with coupons : $e");
        }
    ...
    private static function test_apply_coupon_checkout_cart() : void
    {
        ...
        {
            static::cleanupTestEnviroment($database);
            throw new Exception("Exception while testing applying and checking out with coupons : $e");
        }
    ...
    private static function test_apply_coupon() : void
    {
        ...
        {
            static::cleanupTestEnviroment($database);
            throw new Exception("Exception while testing applying coupons : $e");
        }
    ...
    private static function temp_populate_DB() : void
    {
        try
        {
            ...
            $doesAccountHaveLootForPrice = static::doesAccountHavePriceOfProduct($mockProduct, $mockSellerAccount);
            if(!$doesAccountHaveLootForPrice) throw new Exception("COMPONENT TEST FAILED : seller did not reseve loots for product price");
    ...
    private static function test_Successful_TransactItems() : void
    {
        ...
        {
            static::cleanupTestEnviroment($database);
            throw new Exception("Exception while testing transacting items : $e");
        }
    ...
    private static function testProductCreation() : void
    {
        ...
        try
        {
            ...
            if(!$foundPriceResp->success) throw new Exception("COMPONENT TEST FAILED : getting price failed : $foundPriceResp->message");
    ...
    private static function testCartLootChecks() : void
    {
        ...
        try
        {
            ...
            //Test canAccountAffordItemPriceInCart
            $linkLootsResp = StoreController::linkLootsToProductAsStock($mockProduct, $mockLoots);
            if(!$linkLootsResp->success) throw new Exception("COMPONENT TEST FAILED : failed to link loots to product as stock : $linkLootsResp->message");

            $canAccountAffordItemPriceResp = StoreController::canAccountAffordItemPriceInCart($cartResp->data);
            if(!$canAccountAffordItemPriceResp->success) throw new Exception("COPMONENT TEST FAILED : failed to check if account could afford cart products");
            if($canAccountAffordItemPriceResp->data !== false) throw new Exception("COMPONET TEST FAILED : account returned to be able to afford cart products when it was expected not to : $canAccountAffordItemPriceResp->message");

            static::giveNumberOfRaffleTicketsToAccount($mockBuyerAccount, 3);

            $canAccountAffordItemPriceResp = StoreController::canAccountAffordItemPriceInCart($cartResp->data);
            if(!$canAccountAffordItemPriceResp->success) throw new Exception("COPMONENT TEST FAILED : failed to check if account could afford cart products");
            if($canAccountAffordItemPriceResp->data !== true) throw new Exception("COMPONET TEST FAILED : account returned to not be able to afford cart products when it was expected to : $canAccountAffordItemPriceResp->message");
    ...
    private static function doesCartItemHaveCouponPrice(vCartItem $cartItem, vCoupon $coupon) : bool
    {
        ...
        if(!$result) throw new Exception("result returned false while checking if cart item has coupon price while getting cartItem info");
        ...
        for($i = 0; $i < count($cartItem->price); $i++)
        {
            ...
            foreach($coupon->price as $couponPriceComponent)
            {
                ...
                {
                    $priceComponentFound = true;
                    break;
                }
            }

            if(!$priceComponentFound) throw new Exception("COMPONENT TEST FAILED : Cart item contains price not found in coupon : ".json_encode($priceComponent));
        }
    ...
    private static function test_NotEnoughtLootToBuy_ReserveNonFungibleProduct() : void
    {
        ...
        {
            static::cleanupTestEnviroment($database);
            throw new Exception("Exception while testing reserving non fungible product with not enoguh loot to buy : $e");
        }
    ...
    private static function test_Successful_ReserveNonFungibleProduct() : void
    {
        ...
        {
            static::cleanupTestEnviroment($database);
            throw new Exception("Exception while testing reserving non fungible products : $e");
        }
    ...
    private static function testLinkingFungibleProductsAndLoots() : void
    {
        ...
        {
            static::cleanupTestEnviroment($database);
            throw new Exception("Exception while testing linking fungible products and loots : $e");
        }
    ...
    private static function testLinkingProductsAndLoots() : void
    {
        ...
        {
            static::cleanupTestEnviroment($database);
            throw new Exception("Exception while testing linking products and loots : $e");
        }
    ...
    private static function testCartItemControl() : void
    {
        ...
        {
            //static::cleanupTestEnviroment($database);
            throw new Exception("Exception while testing cart item control : $e");
        }
    ...
    public static function createTestEnviroment(string $uniqueName = '') : string
    {
        ...
        //Price
        static::createPriceComponentTable();
        static::createPriceComponentView();
        ...
        //ProductPriceLink
        static::createProductPriceComponentLinkTable();
        static::createProductPriceComponentLinkView();
        ...
        static::createCartProductPriceComponentLinkTable();
        static::createCartProductPriceComponentLinkView();
        ...
        //Coupon Price Link
        static::createCouponPriceComponentLinkTable();   
    ...
    private static function doesCouponHaveLinksToPrice(vRecordId $coupon, array $price) : bool
    {
        $priceValue = "";
        ...
        $sql = "SELECT cpl.* FROM coupon_price_component_link cpl JOIN ($priceValue) pv WHERE 
        cpl.ref_coupon_ctime = '$coupon->ctime' AND
        cpl.ref_coupon_crand = $coupon->crand AND
        pv.price_component_ctime = cpl.ref_price_component_ctime AND
        pv.price_component_crand = cpl.ref_price_component_crand";


        $result = Database::executeSqlQuery($sql);

        return $result->num_rows === count($price);
    }

    private static function doesAccountHavePriceOfProduct(vProduct $product, vAccount $account) : bool
    {
        $price = $product->price;

        $accountLootAmount = static::returnAmountsOfPriceLootInAccountInventory($price, $account);

        if(count($price) > 0 && count($accountLootAmount) == 0) return false;
        ...
        for($priceComponentIndex = 0; $priceComponentIndex < count($price); $priceComponentIndex++)
        {
            ...
            for($i = 0; $i < count($accountLootAmount); $i++)
            {
                ...
                if(is_null($matchingPrice))
                {
                    ...
                    if($priceComponentIndex === count($price)-1) return false;
                    continue;
                }
    ...
    private static function returnAmountsOfPriceLootInAccountInventory(array $price, vAccount $account) : array
    {
        $whereClause = static::createWhereClauseForReturnAmountsOfPriceLootInAccountInventory($price);
    ...
    private static function createWhereClauseForReturnAmountsOfPriceLootInAccountInventory(array $price) : string
    {
        $whereClause = "";

        for($i = 0; $i < count($price); $i++)
        {
            $whereClause .= "item_id = ".$price[$i]->item->crand;

            if($i != count($price) - 1) $whereClause .= " OR ";
        }
    ...
    public static function returnMockInfiniteOverrideCouponObject(vProduct $product, array $price) : Coupon
    {
        $coupon = new Coupon();
    ...
    public static function returnMockInfiniteCouponObject(vProduct $product, array $price) : Coupon
    {
        $coupon = new Coupon();
    ...
    public static function returnMockProductObject(vStore $store, array $price, vMedia $media) : Product
    {
    ...
    public static function returnMockPriceComponentObject(string $priceObjectName, int $amount = 1) : PriceComponent
    {
        ...
        $price = new PriceComponent($amount, null, $id);

        return $price;
    }

    private static function returnMockPriceCouponObject(vItem $priceItem, int $amount = 1) : PriceComponent
    {
    ...
    public static function returnMockNonFungibleCouponPriceItem(vMedia $media) : Item
    {
        ...
        $item->desc = "Non-fungible Item to be used for replacment of price via coupons";
    ...
    public static function returnMockNonFungibleOverrideCouponPriceItem(vMedia $media) : Item
    {
        ...
        $item->desc = "Non-fungible Item to be used for replacment of price via coupons and overriding other coupons";
    ...
    public static function createMockInfiniteOverrideCoupon(vProduct $product, array $price) : vCoupon
    {
        $coupon = static::returnMockInfiniteOverrideCouponObject($product, $price);
    ...
    public static function createMockInfiniteCoupon(vProduct $product, array $price) : vCoupon
    {
        $coupon = static::returnMockInfiniteCouponObject($product, $price);
    ...
    public static function createMockCouponPrice(vItem $item, int $amount = 1) : array
    {
    ...
    private static function createCartItemView() : void
    {
        ...
        SELECT
            ...
            vpcomponent.ctime as 'price_component_ctime',
            vpcomponent.crand as 'price_component_crand',
            vpcomponent.amount as 'price_component_amount',
            vpcomponent.currency_code as 'price_component_currency_code',
            vpcomponent.item_name as 'price_component_item_name',
            vpcomponent.item_desc as'price_component_item_desc',
            vpcomponent.media_path_small as 'price_component_media_path_small',
            vpcomponent.media_path_large as 'price_component_media_path_large',
            vpcomponent.media_path_back as 'price_component_media_path_back',
            vpcomponent.item_ctime as 'price_component_item_ctime',
            vpcomponent.item_crand as 'price_component_item_crand',
            vpcomponent.item_is_fungible as 'price_component_item_is_fungible',
            vcu.ctime as `coupon_ctime`,
            vcu.crand as `coupon_crand`,
            vcu.code as `coupon_code`,
            vcu.description as `coupon_description`,
        ...
        FROM cart_product_link cplink
            ...
            JOIN v_price_component vpcomponent ON pplink.ref_price_component_ctime = vpcomponent.ctime AND pplink.ref_price_component_crand = vpcomponent.crand
            LEFT JOIN (select * from coupon_cart_product_link where removed = 0 AND checked_out = 0) ccpl ON ccpl.ref_cart_product_link_ctime = cplink.ctime AND ccpl.ref_cart_product_link_crand = cplink.crand 
            LEFT JOIN v_coupon vcu ON vcu.ctime = ccpl.ref_coupon_ctime AND vcu.crand = ccpl.ref_coupon_crand
        );";
    ...
    private static function createPriceComponentTable() : void
    {
        $sql = "create table IF NOT EXISTS price_component
        (
            ctime datetime(6) not null,
            crand bigint not null,
            amount int not null,
            ...
            constraint fk_price_component_ctime_crand_item_ctime_crand FOREIGN KEY (ref_item_crand) REFERENCES item(id)
        );";

        database::executeSqlQuery($sql, []);
    }

    private static function createPriceComponentView() : void
    {
        $sql = "CREATE OR REPLACE VIEW v_price_component AS (
        SELECT 
            p.ctime,
            p.crand,
            p.amount,
    ...
    private static function createCartProductPriceComponentLinkTable() : void
    {
        $sql = "CREATE TABLE IF NOT EXISTS cart_product_price_component_link (
            ctime DATETIME(6) NOT NULL,
            crand BIGINT NOT NULL,  
            ...
            CONSTRAINT fk_cart_product_pcomp_link_cart_product_link_ctime_crand FOREIGN KEY (ref_cart_product_link_ctime, ref_cart_product_link_crand) REFERENCES cart_product_link(ctime, crand),
            CONSTRAINT fk_cart_product_pcomp_link_pcomp_citme_crand FOREIGN KEY (ref_price_component_ctime, ref_price_component_crand) REFERENCES price_component(ctime, crand)
        );";

        database::executeSqlQuery($sql, []);
    }

    private static function createCartProductPriceComponentLinkView() : void
    {
        $sql = "CREATE OR REPLACE VIEW v_cart_product_price_component_link AS
            SELECT
            ctime,
            crand,
    ...
    //PRODUCT PRICE LINK
    private static function createProductPriceComponentLinkTable() : void
    {
        $sql = "create table IF NOT EXISTS product_price_component_link
        (
            ctime datetime(6) not null,
            crand bigint not null,
            ref_product_ctime datetime(6) not null,
            ...
            CONSTRAINT fk_product_pcomp_link_ref_product_cId_product_cId FOREIGN KEY (ref_product_ctime, ref_product_crand) REFERENCES product(ctime, crand),
            CONSTRAINT fk_product_pcomp_link_ref_pcomp_cId_pcomp_cId FOREIGN KEY (ref_price_component_ctime, ref_price_component_crand) REFERENCES price_component(ctime, crand)
        );";

        database::executeSqlQuery($sql, []);
    }

    private static function createProductPriceComponentLinkView() : void
    {
        $sql = "CREATE OR REPLACE VIEW v_product_price_component_link AS (
            SELECT 
                ppl.ctime,
                ppl.crand,
    ...
    private static function createCouponPriceComponentLinkTable() : void
    {

        $query = "CREATE TABLE IF NOT EXISTS coupon_price_component_link(
                ctime DATETIME(6) NOT NULL,
                crand BIGINT NOT NULL,
                ...
                PRIMARY KEY (ctime, crand),
                CONSTRAINT fk_coupon_price_component_link_coupon_ctime_crand FOREIGN KEY coupon(ref_coupon_ctime, ref_coupon_crand) REFERENCES coupon(ctime, crand),
                CONSTRAINT fk_coupon_price_component_link_price_component_ctime_crand FOREIGN KEY price_component(ref_price_component_ctime, ref_price_component_crand) REFERENCES price_component(ctime, crand)
        );";
    ...
    private static function createTransactionTable() : void
    {
        ...
        (
            ...
            void boolean not null default 0,
            ref_price_ctime_crand LONGTEXT not null default '[]',
