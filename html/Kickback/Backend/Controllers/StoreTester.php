...
class StoreTester
{
    ...
    private static function test_apply_override_coupon_checkout_cart() : void
    {
        ...
        try
        {
            ...
            if(count($overrideCoupon->prices) !== count($mockOverrideCouponPrices)) throw new Exception("COMPONENT TEST FAILED : override coupon does not have all of the mockOverrideCouponPrices. Expected : ".count($mockOverrideCouponPrices)." | Actual : ".count($overrideCoupon->prices));
            ...
            if(count($coupon->prices) !== count($mockCouponPrices)) throw new Exception("COMPONENT TEST FAILED : coupon does not have all of the mockCouponPrices. Expected : ".count($mockCouponPrices)." | Actual : ".count($coupon->prices));
            ...
            $mockProduct->prices = $overrideCoupon->prices;
    ...
    private static function test_apply_coupon_checkout_cart() : void
    {
        ...
        try
        {
            ...
            $mockProduct->prices = $coupon->prices;
            $doesAccountHaveLootForPrices = static::doesAccountHavePricesOfProduct($mockProduct, $mockSellerAccount);
            if(!$doesAccountHaveLootForPrices) throw new Exception("COMPONENT TEST FAILED : seller did not receive loots for product prices");
    ...
    private static function doesCartItemHaveCouponPrices(vCartItem $cartItem, vCoupon $coupon) : bool
    {
        ...
        for($i = 0; $i < count($cartItem->prices); $i++)
        {
            $price = $cartItem->prices[$i];

            $priceFound = false;
            foreach($coupon->prices as $couponPrice)
            {
                if($price->ctime === $couponPrice->ctime && $price->crand === $couponPrice->crand)
                {
                    $priceFound = true;
                    break;
                }
