...
try
{
    ...
    $response = null;
    $response_code = StoreService::add_product_to_cart_by_locator($sessionAccount, $request_contents_json, $response);
    if ( $response_code !== 0 ) {
...
catch( \Throwable $e )
{
    ...
    $response = new Response(false,
        'Failed to add product to cart: ' . ThrowableOverrides::message($e),
        $e->__toString());
}
finally {
    Endpoint::end();
