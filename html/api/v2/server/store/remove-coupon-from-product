...
try
{
    ...
    $response = null;
    $response_code = StoreService::remove_coupon_from_cart_product($sessionAccount, $request_contents_json, $response);
    if ( $response_code !== 0 ) {
...
catch( \Throwable $e )
{
    ...
    $response = new Response(false,
        'Failed to remove coupon from product: ' . ThrowableOverrides::message($e),
        $e->__toString());
}
finally {
    Endpoint::end();
