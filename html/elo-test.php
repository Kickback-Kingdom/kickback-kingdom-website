...
$result = mysqli_query($conn, 'SELECT game_match.Id, game_id, game.name as game_name, game.MinRankedMatches FROM game_match left join game on game_match.game_id = game.Id');
if (!$result) {
...
foreach ($matches as $match) {
    ...
    $stmt = mysqli_prepare($conn, 'SELECT account_id, win, team_name, account.Username FROM game_record left join account on game_record.account_id = account.Id WHERE game_match_id = ? AND game_id = ?');
    if (!$stmt) {
        die("Error preparing statement: " . mysqli_error($conn));
    }
