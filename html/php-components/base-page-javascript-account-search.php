...
function SearchForAccount(formId, pageIndex = 1, clickableFunction = null)
{
...
function OnSelectAccountChangeSearchParams(formId)
{
    console.log("search for Account!");
    SearchForAccount(formId, 1);
}
...
for (let i = 0; i < badges.length && i < 5; i++) {
    let badge = badges[i];
    badgeCode += `<span tabindex="0" data-bs-toggle="popover" data-bs-custom-class="custom-popover" data-bs-trigger="focus" data-bs-placement="top" data-bs-title="${badge.name}" data-bs-content="${badge.desc}"><img src="/assets/media/${badge.SmallImgPath}" class="loot-badge"></span>`;
}
...
${clickableLayer}
        ...
        <div class="card-header${isRanked1 ? " ranked-1" : ""}">
            <h5 class="player-card-name">
                ...
                <span> Level ${playerCardAccount.level}
                    <div class="progress" role="progressbar" aria-label="Animated striped example" aria-valuenow="${playerCardAccount.exp_current}" aria-valuemin="0" aria-valuemax="${playerCardAccount.exp_goal}" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="${playerCardAccount.exp_current * 10}/${playerCardAccount.exp_goal * 10} EXP">
                        <div class="progress-bar ${isRanked1 ? "bg-progress-ranked-1" : "bg-secondary"} progress-bar-striped progress-bar-animated" style="width: ${(playerCardAccount.exp_current / playerCardAccount.exp_goal) * 100}%">
                        </div>
                    </div>
                </span>
            </h5>
        ...
        <div class="card-body align-items-start d-flex justify-content-start${isRanked1 ? " ranked-1" : ""}">
            <img class="img-fluid img-thumbnail" src="/assets/media/${playerCardAccount.avatar_media}" />
